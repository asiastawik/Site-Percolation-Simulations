...
with open(str(file), 'w') as f:
    ...
    for p_range_val, color in zip([p_range, p_range2, p_range3], ['black', 'blue', 'red']):
...
sizes = [7, 4, 2]
colors = []
for p_range_val, markers, size, color in zip([p_range3, p_range2, p_range], ['d', 's', 'o'], sizes, ['0.8', '0.5', '0.2']):
    Pflow_list = []
    p_vals =  np.arange(p_range_val[0], p_range_val[1] + p_range_val[2], p_range_val[2])
    for p in p_vals:
        count = 0
        for t in range(T):
            trial_A = copy.copy(A)
    ...
    ax.plot(p_vals, Pflow_list, 'o-', marker = markers, markersize = size, color=color, linestyle='None', zorder=2, alpha=0.8,  label='p range: {} to {}'.format(p_range_val[0], p_range_val[1]))
    #ax.scatter(np.arange(p_range_val[0], p_range_val[1] + p_range_val[2], p_range_val[2]), Pflow_list, s=sizes, marker=markers, color=color, zorder=2)
    
ax.set_xlabel('p')
ax.set_ylabel('Pflow')
ax.set_title('Site Percolation on Square Lattice')
ax.legend(loc='best')
plt.show()
